...
use vm_memory::{GuestAddress, GuestAddressSpace, GuestMemoryMmap};
    ...
    B: NewBitmap + Clone,
{
    ...
    fn set_mem_table(
        ...
        for (region, file) in ctx.iter().zip(files) {
            regions.push(region.as_guest_region(file)?);
        }
    ...
    fn add_mem_region(
        ...
        let guest_region = Arc::new(region.as_guest_region(file)?);
