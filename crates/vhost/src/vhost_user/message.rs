...
use vm_memory::{ByteValued, FileOffset, GuestAddress, GuestRegionMmap};
...
impl VhostUserMemoryRegion {
    ...
    pub fn as_range(&self, file: File) -> (GuestAddress, usize, Option<FileOffset>) {
        let g_addr = GuestAddress(self.guest_phys_addr);
        let len = self.memory_size as usize;
        let f_off = FileOffset::new(file, self.mmap_offset);

        (g_addr, len, Some(f_off))
    }

    /// Creates range from Self.
    pub fn as_guest_region<B: NewBitmap>(&self, file: File) -> Result<GuestRegionMmap<B>> {
        let range = self.as_range(file);

        GuestRegionMmap::from_range(range.0, range.1, range.2)
...
impl VhostUserMemoryRegion {
    ...
    /// Creates range from Self.
    pub fn as_range(&self, file: File) -> (GuestAddress, usize, Option<FileOffset>, u32, u32) {
        let g_addr = GuestAddress(self.guest_phys_addr);
        let len = self.memory_size as usize;
        let f_off = Some(FileOffset::new(file, self.mmap_offset));

        (g_addr, len, f_off, self.xen_mmap_flags, self.xen_mmap_data)
    }
    ...
    pub fn as_guest_region<B: NewBitmap>(&self, file: File) -> Result<GuestRegionMmap<B>> {
        ...
        GuestRegionMmap::from_range(range.0, range.1, range.2, range.3, range.4)
            .map_err(|e| Error::ReqHandlerError(io::Error::new(io::ErrorKind::Other, e)))
    }
    ...
    fn is_valid(&self) -> bool {
        if !self.validate() {
            ...
            MmapXenFlags::is_valid(self.xen_mmap_flags)
        }
    }
}
...
impl VhostUserSingleMemoryRegion {
    ...
    /// Creates range from Self.
    pub fn as_guest_region<B: NewBitmap>(&self, file: File) -> Result<GuestRegionMmap<B>> {
        self.region.as_guest_region(file)
    }
}
